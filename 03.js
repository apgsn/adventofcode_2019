...
const manhattans = []; delays = [];

const mapWire = (wire) => {
  const board = {};
  let x = 0; y = 0; steps = 1;

  wire.split(',').forEach(el => {
    const letter = el.slice(0, 1);
    const isVert = letter === 'U' || letter === 'D';
    const dir = letter === 'R' || letter === 'D' ? 1 : -1;
    const len = el.slice(1);

    for (let i = 1; i <= len; i++ , steps++) {
      const coordX = x + i * dir * isVert;
      const coordY = y + i * dir * !isVert;
      board[`${coordX}_${coordY}`] |= steps;
    }

    isVert ? x += dir * len : y += dir * len;
  });

  return board;
}

const board1 = mapWire(wire1);
const board2 = mapWire(wire2);

Object.keys(board1)
  .filter(coord => board2[coord])
  .forEach(coord => {
    manhattans.push(coord.split('_').reduce((acc, n) => acc + Math.abs(n), 0));
    delays.push(board1[coord] + board2[coord]);
  });

console.log(`Part one: ${Math.min(...manhattans)}`);
console.log(`Part two: ${Math.min(...delays)}`);
